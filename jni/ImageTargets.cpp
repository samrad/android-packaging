...
Java_com_packagingmidsweden_packaging_ImageTargets_getOpenGlEsVersionNative(JNIEnv *, jobject)
{
#ifdef USE_OPENGL_ES_1_1        
    return 1;
#else
...
Java_com_packagingmidsweden_packaging_ImageTargets_setActivityPortraitMode(JNIEnv *, jobject, jboolean isPortrait)
{
...
JNIEXPORT void JNICALL
Java_com_packagingmidsweden_packaging_ImageTargets_switchDatasetAsap(JNIEnv *, jobject)
{
...
JNIEXPORT int JNICALL
Java_com_packagingmidsweden_packaging_ImageTargets_initTracker(JNIEnv *, jobject)
{
...
JNIEXPORT void JNICALL
Java_com_packagingmidsweden_packaging_ImageTargets_deinitTracker(JNIEnv *, jobject)
{
...
JNIEXPORT int JNICALL
Java_com_packagingmidsweden_packaging_ImageTargets_loadTrackerData(JNIEnv *, jobject)
{
...
JNIEXPORT int JNICALL
Java_com_packagingmidsweden_packaging_ImageTargets_destroyTrackerData(JNIEnv *, jobject)
{
...
JNIEXPORT void JNICALL
Java_com_packagingmidsweden_packaging_ImageTargets_onQCARInitializedNative(JNIEnv *, jobject)
{
...
JNIEXPORT void JNICALL
Java_com_packagingmidsweden_packaging_ImageTargetsRenderer_renderFrame(JNIEnv *env, jobject obj)
{
...
//    glEnable(GL_CULL_FACE);
    ...
    jmethodID method = env->GetMethodID(javaClass, "handleTrackable", "(Ljava/lang/String;[F)V");

//    // For target size
//    jfloatArray distanceArray = env->NewFloatArray(3);
//    jmethodID distanceMethod = env->GetMethodID(javaClass, "targetDistance", "([F)V");
    ...
    for(int tIdx = 0; tIdx < state.getNumActiveTrackables(); tIdx++)
    {
        ...
        jstring trackableName = env->NewStringUTF(trackable->getName());
        QCAR::Matrix44F modelViewMatrix = QCAR::Tool::convertPose2GLMatrix(trackable->getPose());
        QCAR::Matrix44F modelViewMatrix1 = modelViewMatrix;

        // Rotate the ModelView matrix 90 degree to change
        // coordinates from right-handed to jPCT (upside-down)
        ...
        env->CallObjectMethod(obj, method, trackableName, modelviewArray);
        env->DeleteLocalRef(modelviewArray);
        env->DeleteLocalRef(trackableName);

        // Getting the target dimension
...
//        QCAR::Vec3F position(modelViewMatrix.data[12], modelViewMatrix.data[13], modelViewMatrix.data[14]);
        ...
        // Passing distance array
//        env->SetFloatArrayRegion(distanceArray, 0, 3, position.data);
//        env->CallObjectMethod(obj, distanceMethod, distanceArray);
//        env->DeleteLocalRef(distanceArray);
...
JNIEXPORT void JNICALL
Java_com_packagingmidsweden_packaging_ImageTargets_initApplicationNative(
...
JNIEXPORT void JNICALL
Java_com_packagingmidsweden_packaging_ImageTargets_deinitApplicationNative(
...
JNIEXPORT void JNICALL
Java_com_packagingmidsweden_packaging_ImageTargets_startCamera(JNIEnv *, jobject)
{
...
//    // Just for test
//    char result[50];
//    sprintf(result, "ViewPortWidth : %d | ViewportHeight : %d ", viewportWidth, viewportHeight);
//    LOG(result);
...
JNIEXPORT void JNICALL
Java_com_packagingmidsweden_packaging_ImageTargets_stopCamera(JNIEnv *, jobject)
{
...
JNIEXPORT void JNICALL
Java_com_packagingmidsweden_packaging_ImageTargets_setProjectionMatrix(JNIEnv *, jobject)
{
...
Java_com_packagingmidsweden_packaging_ImageTargets_activateFlash(JNIEnv*, jobject, jboolean flash)
{
...
JNIEXPORT jboolean JNICALL
Java_com_packagingmidsweden_packaging_ImageTargets_autofocus(JNIEnv*, jobject)
{
...
JNIEXPORT jboolean JNICALL
Java_com_packagingmidsweden_packaging_ImageTargets_setFocusMode(JNIEnv*, jobject, jint mode)
{
    int qcarFocusMode;
...
JNIEXPORT void JNICALL
Java_com_packagingmidsweden_packaging_ImageTargetsRenderer_initRendering(JNIEnv* env, jobject obj)
{
...
Java_com_packagingmidsweden_packaging_ImageTargetsRenderer_updateRendering(
                        JNIEnv* env, jobject obj, jint width, jint height)
{
    ...
    // Pass the camera resolution to Java for creating the jPCT framebuffer object
    QCAR::VideoBackgroundConfig config = QCAR::Renderer::getInstance().getVideoBackgroundConfig();

    jintArray resolution;
    resolution = env->NewIntArray(2);
    ...
    env->SetIntArrayRegion(resolution, 0, 2, config.mSize.data);
     return resolution;
}

// To be used in OpenGL Renderer class
JNIEXPORT jfloatArray JNICALL
Java_com_packagingmidsweden_packaging_ImageTargetsRenderer_getFOV(JNIEnv *env, jobject obj)
{
    LOG("Java_com_qualcomm_QCARSamples_ImageTargets_ImageTargets_getProjectionMatrix");

    // Result FOVs (to be passed)
    jfloatArray arrayFOV;
    arrayFOV = env->NewFloatArray(2);

    // Temporary FOV (holding the values)
    float tempFOV[2];


//    float horizentalFOV = 0;

    // Cache the projection matrix:
    const QCAR::CameraCalibration& cameraCalibration = QCAR::CameraDevice::getInstance().getCameraCalibration();
    projectionMatrix = QCAR::Tool::getProjectionGL(cameraCalibration, 2.0f,2000.0f);

    // Size and Focal length for calculating the Field of View (FOV)
    QCAR::Vec2F sizeVector = cameraCalibration.getSize();
    QCAR::Vec2F focalVector = cameraCalibration.getFocalLength();

    // Calculation of FOVs
    tempFOV[0] = 2 * atan ((0.5 * sizeVector.data[0]) / focalVector.data[0]);
    tempFOV[1] = 2 * atan ((0.5 * sizeVector.data[1]) / focalVector.data[1]);


    // Passing the Projection matrix up to Java
//    jclass javaClass = env->GetObjectClass(obj);
//    jfloatArray projectionArray = env->NewFloatArray(16);
//    env->SetFloatArrayRegion(projectionArray, 0, 16, projectionMatrix.data);
//    jmethodID method = env->GetMethodID(javaClass, "updateProjectionMatrix", "([F)V");
//    env->CallObjectMethod(obj, method, projectionArray);
//    env->DeleteLocalRef(projectionArray);

    env->SetFloatArrayRegion(arrayFOV, 0, 2, tempFOV);
    return arrayFOV;
}
